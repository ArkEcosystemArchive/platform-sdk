...
export class EntityHistoryAggregate {
	...
	public async all(registrationId: string): Promise<ExtendedTransactionData[]> {
		const registrations = [
			transformTransactionData(this.#wallet, await this.#wallet.client().transaction(registrationId)),
		];
		const updates = await this.aggregate(registrationId, "resign");
		const resignations = await this.aggregate(registrationId, "update");

		return [...registrations, ...updates.items(), ...resignations.items()];
	}

	private async aggregate(registrationId: string, entityAction: string): Promise<ExtendedTransactionDataCollection> {
