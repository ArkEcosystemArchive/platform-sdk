...
it.only("should throw if a transaction is retrieved that does not exist", async () => {
	const publicKeys = [
		"02edf966159de0013ca5b99371c5436e78f22df0d565eceee09feb977fe49cb910",
		"0205d9bbe71c343ac9a6a83a4344fd404c3534fc7349827097d0835d160bc2b896",
	];

	const signatory = await wallet.coin().signatory().mnemonic("this is a top secret passphrase");
	...
	const { accepted, rejected, errors } = await wallet.transaction().broadcast(uuid);
	console.log(accepted, rejected, errors)
	// const transactionId = accepted[0];

	// await wallet.transaction().sync();
	// await wallet.transaction().addSignature(transactionId, signatory); // use mnemonic or wif signatory
