...
let MessageService = class MessageService extends platform_sdk_1.Services.AbstractMessageService {
	async sign(input) {
		try {
			return {
				...
				signatory: crypto_1.privateToPublic(input.signatory.signingKey()),
				signature: crypto_1.sign(input.message, input.signatory.signingKey()),
			};
		} catch (error) {
	...
	async verify(input) {
		try {
			return crypto_1.verify(input.signature, input.message, input.signatory);
		} catch (error) {
			throw new platform_sdk_1.Exceptions.CryptoException(error);
		}
	}
