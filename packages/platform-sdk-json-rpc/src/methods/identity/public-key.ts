...
export const registerPublicKey = () => [
	{
		...
		async method({ coin, network, mnemonic }) {
			return (await makeCoin({ coin, network })).publicKey().fromMnemonic(mnemonic);
		},
	...
	},
	{
		...
		async method({ coin, network, min, publicKeys }) {
			return (await makeCoin({ coin, network })).publicKey().fromMultiSignature(min, publicKeys);
		},
		schema: Joi.object({
			...baseSchema,
	...
	},
	{
		...
		async method({ coin, network, wif }) {
			return (await makeCoin({ coin, network })).publicKey().fromWIF(wif);
		},
