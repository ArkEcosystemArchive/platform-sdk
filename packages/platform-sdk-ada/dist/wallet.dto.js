...
class WalletData extends platform_sdk_1.DTO.AbstractWalletData {
	...
	publicKey() {
		throw new platform_sdk_1.Exceptions.NotImplemented(this.constructor.name, this.publicKey.name);
	}
	balance() {
		var _a, _b;
		return {
			available: platform_sdk_support_1.BigNumber.make(
				(_a = this.data.balance.available) === null || _a === void 0 ? void 0 : _a.quantity,
			),
			fees: platform_sdk_support_1.BigNumber.make(
				(_b = this.data.balance.reward) === null || _b === void 0 ? void 0 : _b.quantity,
			),
		};
	}
	nonce() {
		throw new platform_sdk_1.Exceptions.NotImplemented(this.constructor.name, this.nonce.name);
	}
	secondPublicKey() {
