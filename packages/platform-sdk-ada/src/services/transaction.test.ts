	...
	describe("#transfer", () => {
		...
		it("public key not matching mnemonic", () => {
			...
			expect(async () => subject.transfer({
				...
				.rejects
				.toThrow("Public key doesn't match the given mnemonic");
		});
		it("no mnemonic", () => {
			expect(async () => subject.transfer({
				from:
				...
				.rejects
				.toThrow("Method TransactionService#transfer expects the argument [sign.mnemonic] but it was not given");
		});
	});
});
