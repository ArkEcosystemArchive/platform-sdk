...
export const getCip1852Account = (mnemonic: string, slip44: number): CardanoWasm.Bip32PrivateKey => CardanoWasm.Bip32PrivateKey.from_bip39_entropy(
		Buffer.from(BIP39.toEntropy(mnemonic), "hex"),
		Buffer.from(""),
	).derive(harden(Purpose.CIP1852)).derive(harden(slip44)).derive(harden(0));
