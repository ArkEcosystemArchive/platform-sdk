...
import { deriveAccountKey, deriveChangeKey, deriveRootKey, deriveStakeKey, deriveUtxoKey } from "./transaction.helpers";
import { addressFromMnemonic } from "../crypto/shelley/address";
import { SHELLEY_COIN_TYPE } from "../crypto/shelley/constants";
...
export class AddressesHelper {
	...
	constructor(mnemonic: string, pageSize: number = 20) {
		this.#pageSize = pageSize;
		this.#networkId = "1";
		this.#rootKeyPair = deriveRootKey(mnemonic);
		this.#addressesForAccount = [];

		const account0Key = deriveAccountKey(this.#rootKeyPair, SHELLEY_COIN_TYPE, 0);
