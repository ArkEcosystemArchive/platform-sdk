...
let AddressService = class AddressService extends platform_sdk_1.Services.AbstractAddressService {
	...
	async fromMnemonic(mnemonic, options) {
		...
		return this.fromPublicKey(
			platform_sdk_crypto_1.BIP44.deriveChild(mnemonic, {
	...
	async fromPublicKey(publicKey, options) {
		try {
			return { type: "rfc6979", address: ripple_keypairs_1.deriveAddress(publicKey) };
		} catch (error) {
	...
	async fromSecret(secret) {
		return {
			type: "rfc6979",
			address: ripple_keypairs_1.deriveAddress(ripple_keypairs_1.deriveKeypair(secret).publicKey),
		};
	}
