...
const hw_app_xrp_1 = __importDefault(require("@ledgerhq/hw-app-xrp"));
let LedgerService = class LedgerService extends platform_sdk_1.Services.AbstractLedgerService {
	...
	async connect(transport) {
		...
		__classPrivateFieldSet(
			...
			new hw_app_xrp_1.default(__classPrivateFieldGet(this, _LedgerService_ledger, "f")),
			"f",
		);
	}
	async disconnect() {
	...
	async signTransaction(path, payload) {
		return __classPrivateFieldGet(this, _LedgerService_transport, "f").signTransaction(path, payload);
	}
};
