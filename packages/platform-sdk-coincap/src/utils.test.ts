...
test("`rates` object does not contain either `from` or `to` currency!", async () => {
	expect(() => convertToCurrency(10, { from: 'ARK', to: 'BTC', base: null, rates: { BTC: 0.1 } })).toThrow("`rates` object does not contain either `from` or `to` currency!");
});

test("Please specify the `from` and/or `to` currency or use parsing!", async () => {
	expect(() => convertToCurrency(10, { from: null, to: null, base: null, rates: [] })).toThrow("Please specify the `from` and/or `to` currency or use parsing!");
});
