...
let LedgerService = class LedgerService extends platform_sdk_1.Services.AbstractLedgerService {
	...
	async connect(transport) {
		...
		__classPrivateFieldSet(
			...
			new hw_app_str_1.default(__classPrivateFieldGet(this, _LedgerService_ledger, "f")),
			"f",
		);
	}
	async disconnect() {
	...
	async getPublicKey(path) {
		const { publicKey } = await __classPrivateFieldGet(this, _LedgerService_transport, "f").getPublicKey(
			path,
			true,
			true,
		);
	...
	async signTransaction(path, payload) {
		const { signature } = await __classPrivateFieldGet(this, _LedgerService_transport, "f").signTransaction(
			path,
			payload,
		);
